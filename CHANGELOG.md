...
- `--retain-schedules` for `pv-bess sensitivity`, keeping the schedule behind every row: each distinct run is published as `schedules/<analysis_input_sha256>.csv`, byte for byte the `dispatch.csv` that `pv-bess run` writes for that row's scenario and financial assumptions, and every row names its file in a `schedule_file` field of `sensitivity.json` and a last `schedule_file` column of `sensitivity.csv`, relative to the output directory. Files are named by the analysis hash, so rows with the same analysis inputs share one and no two files are identical; a financial-only row keeps its own file over the base dispatch. The table and `schedules/` are published together or not at all: an existing `schedules/` is refused without `--force` and replaced whole with it, and a run without the flag leaves it alone. Without the flag nothing changes, and with it no number does.
