...
- `--retain-schedules` for `pv-bess sensitivity`, keeping the schedule behind every row: each distinct run is published as `schedules/<analysis_input_sha256>.csv`, byte for byte the `dispatch.csv` that `pv-bess run` writes for that row's scenario and financial assumptions at the same `--time-limit` and `--mip-gap`, and every row names its file in a `schedule_file` field of `sensitivity.json` and a last `schedule_file` column of `sensitivity.csv`, relative to the output directory. Files are named by the analysis hash, so rows with the same analysis inputs share one and no two files are identical; a financial-only row keeps its own file over the base dispatch. The table and `schedules/` are published together or not at all when a write or rename fails: an existing `schedules/` is refused without `--force` and replaced whole with it, except for a read-only file inside it that also has a hard link elsewhere, which is left alone, the old directory, or what remains of it, then staying beside the new output as a hidden `.schedules.*.backup.tmp`; a run without the flag leaves it alone. Without the flag nothing changes, and with it no number does.

### Changed

- `pv-bess validate` now refuses two scenarios it previously accepted, with the same message the run produces, because both refusals are decidable from the inputs alone: a `terminal_soc_fraction` different from `initial_soc_fraction` (the financial layer requires equality; previously discovered only after a full solve), and a fade projection whose calendar component alone breaches `minimum_capacity_fraction` within the project life — decidable whatever the cycling fade is, since cycling only adds to the rate, with the pre-solve message reporting the calendar-only year and fraction that the full trajectory breaches at or before. `run`, `sensitivity` (including per-variant, since a capacity variant rescales the SOC endpoints in kWh), and the API refuse the same preconditions before solving instead of after — same error type, text, and status, only earlier — so the most expensive operation in the product is never spent discovering an input-decidable refusal. A consequence worth naming: a scenario that passes `validate` can no longer be mathematically infeasible — every structural infeasibility requires a terminal target different from the initial SOC, and with equality the idle battery satisfies every constraint. The solver's infeasibility message can still appear when it fails numerically on extreme-but-valid magnitudes; that residual path is the solver-numerical-failure class `validate` declares.
